...
function setcountry() {
  var City_business_trip = EdocsApi.getAttributeValue("City_business_trip").value;
  var country1 = EdocsApi.getAttributeValue("country1").value;
  if (City_business_trip && country1) {
    EdocsApi.setAttributeValue({ code: "country", value: `м.${City_business_trip}, ${country1}`, text: null });
  } else if (City_business_trip && !country1) {
    EdocsApi.setAttributeValue({ code: "country", value: `м.${City_business_trip}`, text: null });
  } else if (!City_business_trip && country1) {
    EdocsApi.setAttributeValue({ code: "country", value: `${country1}`, text: null });
  } else {
    EdocsApi.setAttributeValue({ code: "country", value: "", text: null });
  }
}

function onChangeCity_business_trip() {
  setcountry();
}

function onChangecountry1() {
  setcountry();
}
